...
const User = {
  ...
  register: async (
    ...
    try {
      ...
      if (NodeEnv === "development") console.error("Register error:", error);
      throw error;
    }
  },
  ...
  login: async (userData: FormData): Promise<ApiResponse<UserData> | null> => {
    try {
      ...
      if (NodeEnv === "development") console.error("Login error:", error);
      throw error;
    }
  },
  ...
  get: async (email: string): Promise<UserData | null> => {
    try {
      ...
      if (NodeEnv === "development") console.error("Get User error:", error);
      return null;
    }
  },
};
